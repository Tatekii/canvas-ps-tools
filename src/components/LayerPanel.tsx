    ...
    <div className={`bg-gray-800 text-white p-4 w-80 ${className || ''}`}>
      ...
      <div className="space-y-2">
        {layers.length === 0 ? (
          <div className="text-gray-400 text-center py-8">
            No layers yet. Click + to add a test layer.
          </div>
        ) : (
          layers
            .slice() // 创建副本以避免直接修改
            ...
            .map((layer) => (
              <div
                ...
                className={`
                  ...
                  ${dragHandleHoveredLayerId === layer.id && !layer.locked && editingLayerId !== layer.id 
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div 
                    ...
                    className={`
                      p-1 rounded transition-colors cursor-move
                      ${layer.locked || editingLayerId === layer.id 
                  ...
                  >
                    {layer.visible ? <Eye size={16} /> : <EyeOff className='text-yellow-400' size={16} />}
                  </button>
                  ...
                  >
                    {layer.locked ? <Lock size={16} className='text-red-400' /> : <Unlock size={16} />}
                  </button>
                ...
                <div className="mt-2 text-xs text-gray-400">
                  <div>Size: {layer.displayWidth} × {layer.displayHeight}</div>
      ...
      {layers.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-600 text-xs text-gray-400">
          ...
          {activeLayerId && (
            <div>Active: {layers.find(l => l.id === activeLayerId)?.name}</div>
          )}
        </div>
      )}
    </div>
